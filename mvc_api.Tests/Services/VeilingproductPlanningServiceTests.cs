...
public class VeilingproductPlanningServiceTests
{
    ...
    public async Task ScheduleAsync_WithActiveProduct_PersistsPlanning()
    {
        ...
        repo.Setup(r => r.FindAsync(5, It.IsAny<CancellationToken>())).ReturnsAsync(product);
    ...
    public async Task ScheduleAsync_WhenProductInactiveOrAlreadyPlanned_ReturnsFalse()
    {
        ...
        repo.SetupSequence(r => r.FindAsync(6, It.IsAny<CancellationToken>()))
            ...
            .ReturnsAsync(new Veilingproduct { VeilingProductNr = 6, Status = ModelStatus.Active, Startprijs = 10 });

        var service = new VeilingproductPlanningService(repo.Object);

        var inactiveResult = await service.ScheduleAsync(6, startPrijs: 50, veilingNr: 201);
        var alreadyPlannedResult = await service.ScheduleAsync(6, startPrijs: 50, veilingNr: 201);

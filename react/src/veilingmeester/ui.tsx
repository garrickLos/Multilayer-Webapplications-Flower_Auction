import React, {
    memo, useCallback, useDeferredValue, useEffect, useId, useMemo, useRef, useState,
} from "react";
...
export function DataTableInner<T extends RowBase>({
    ...
    const filtered = useMemo(
        () => (!dq ? rows : rows.filter((r) => cols.some((c) => S(c.accessor ? c.accessor(r) : (r[c.key as keyof T] as unknown)).toLowerCase().includes(dq)))),
        [rows, dq, cols]
    );
            ...
            <div className="card shadow-sm border-0 border border-success-subtle">
                ...
                <div className="card-body pt-2">
                    <div className="d-flex align-items-center justify-content-between mb-2">
                        <span className="badge bg-success-subtle text-success" aria-live="polite">{data.length.toLocaleString("nl-NL")} resultaten</span>
                        <div className="input-group input-group-sm" style={{ maxWidth: 320 }}>
                            <span className="input-group-text bg-success-subtle text-success border-success-subtle">Zoek</span>
                            <input className="form-control" value={q} onChange={(e) => setQ(e.target.value)} placeholder={filterPlaceholder} aria-label="Filter resultaten" inputMode="search" autoComplete="off" />
                    ...
                    <div className="table-responsive rounded-3 border bg-body">
                        <table className="table table-sm table-striped table-hover align-middle caption-top" aria-describedby={caption ? `${tableId}-caption` : undefined}>
                                ...
                                {cols.map((c) => {
                                    ...
                                    return (
                                        <th key={c.key} scope="col" aria-sort={active ? (sortDir === "asc" ? "ascending" : "descending") : "none"}
                                            className={cx("text-nowrap text-success", c.className, c.hideSm && "d-none d-md-table-cell", c.sortable && "user-select-none")}
                                            style={c.width != null ? ({ width: c.width } as const) : undefined}
                                            title={c.sortable ? "Klik om te sorteren" : undefined}>
                                            {c.sortable ? (
                                                <button type="button" className="btn btn-link p-0 text-success text-decoration-none d-inline-flex align-items-center gap-1"
                                                        onClick={() => toggleSort(c.key)} aria-label={`Sorteer op ${String(c.header ?? c.key)}`}>
                                                    {c.header ?? c.key}{active && <span className="small">{sortDir === "asc" ? "▲" : "▼"}</span>}
                                                </button>
                            ...
                            {data.map((row, i) => (
                                <tr
                                    ...
                                    onKeyDown={(e) => { if (!onRowClick) return; if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onRowClick(row); }}}
                                >
                                    {cols.map((c) => {
                                        const v: unknown = c.accessor ? c.accessor(row) : (row[c.key as keyof T] as unknown);
                                        return (
                                            <td key={c.key} className={cx("text-truncate", c.className, c.hideSm && "d-none d-md-table-cell")} style={{ maxWidth: 0 }}>
...
export const Modal: React.FC<ModalProps> = ({ title, onClose, children, size, fullscreenUntil, maxWidthPx, autoFocusSelector }) => {
    ...
    useEffect(() => {
        ...
        const id = window.setTimeout(() => (ref.current?.querySelector(autoFocusSelector ?? "button.btn-close") as HTMLElement | null)?.focus?.(), 0);
        return () => { window.clearTimeout(id); window.removeEventListener("keydown", onKey); document.body.style.overflow = oldOverflow; prevFocus.current?.focus?.(); };
    }, [onClose, autoFocusSelector]);

    const dialogCls = cx("modal-dialog modal-dialog-centered modal-dialog-scrollable",
    ...
    const modalNode = (
        <div className="modal show d-block" style={{ zIndex: 1055 }} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby={titleId} ref={ref}>
    ...
    const backdropNode = <div className="modal-backdrop show" style={{ zIndex: 1050 }} aria-hidden="true" onMouseDown={onClose} />;
...
const StatusBadge = ({ status }: { status?: string | null }) => {
    ...
    const name = s.includes("actief") ? "bg-success-subtle text-success"
        : s.includes("afgesloten") ? "bg-secondary-subtle text-secondary"
            : s.includes("gepland") ? "bg-info-subtle text-info"
                : "bg-light text-body";
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!ctl.signal.aborted) { console.error(e); setRowsRaw([]); setError("Kon veilingen niet ophalen."); }
            }
        })();
        return () => ctl.abort();
    }, [productId]);
                        ...
                        <article className="card shadow-sm border-0 border border-success-subtle">
                            {sel?.afbeelding && (
                                <div className="ratio ratio-16x9">
                                    <img src={sel.afbeelding} alt={sel.product?.naam || "product"} className="w-100 h-100 object-fit-cover rounded-top"
                            ...
                            <div className="card-body">
                                ...
                                {sel && (
                                    <ul className="list-group list-group-flush">
                                        <li className="list-group-item d-flex justify-content-between align-items-center px-0"><span className="text-muted">Startprijs</span><strong>{fmtEur(sel.product?.startprijs)}</strong></li>
                                        <li className="list-group-item d-flex justify-content-between align-items-center px-0"><span className="text-muted">Voorraad</span><span>{sel.product?.voorraad ?? ""}</span></li>
                                        <li className="list-group-item d-flex justify-content-between align-items-center px-0"><span className="text-muted">Begintijd</span><span>{sel.begintijd ? fmtDate(new Date(sel.begintijd)) : ""}</span></li>
                                        <li className="list-group-item d-flex justify-content-between align-items-center px-0"><span className="text-muted">Eindtijd</span><span>{sel.eindtijd ? fmtDate(new Date(sel.eindtijd)) : ""}</span></li>
                                    </ul>
                                )}
                            </div>
                        </article>
